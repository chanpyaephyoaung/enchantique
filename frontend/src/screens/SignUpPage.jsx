...
const SignUpPage = () => {
   ...
   const submitHandler = async (e) => {
      e.preventDefault();
      if (!username || !email || !password || !confirmPassword || !telephoneNum) {
         setError("All fields are required to fill up.");
      } else if (password !== confirmPassword) {
         setError("Passwords do not match. Please try again.");
      } else {
         try {
            const res = await signUp({ name: username, email, password, telephoneNum }).unwrap();
      ...
      <Container type="page">
         <FormContainer>
            ...
            <p className="text-clr-black-faded text-xs md:text-sm">
               {`Already a member? `}
               <Link
                  to={redirectPath ? `/signin?redirect=${redirectPath}` : "/signin"}
